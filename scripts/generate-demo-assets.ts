...
function fontPath(): string {
  const candidates = ["C:/Windows/Fonts/NotoSansSC-VF.ttf", "C:/Windows/Fonts/Deng.ttf", "C:/Windows/Fonts/msyh.ttc", "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc", "/System/Library/Fonts/PingFang.ttc"];
  const found = candidates.find(existsSync);
  if (!found) throw new Error("未找到可生成中文演示 PDF 的字体");
  return found;
}
...
async function createAgreement(path: string): Promise<void> {
  const bytes = await new Promise<Buffer>((resolve, reject) => {
    ...
    doc.font(fontPath()).fillColor("#b34d27").fontSize(11).text(DEMO_MARK).moveDown();
