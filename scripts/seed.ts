...
const now=new Date().toISOString();const eligibility=JSON.stringify({eligible:true,reasons:["虚构健身预付服务演示案例"],suggestedMaterials:["付款记录","商家主体信息","至少一份退费沟通","服务约定"],safetyNotice:null,boundary:"只整理材料，不提供法律结论。"});const detail=(value:unknown)=>({value,state:"USER_CONFIRMED",evidenceSupport:"USER_STATEMENT",savedAt:now});const details=JSON.stringify({merchantLegalName:detail("星云健康管理有限公司（虚构）"),storeName:detail("星云健身天府店（虚构）"),serviceName:detail("健身年卡"),paymentAmountYuan:detail(2999),paymentDate:detail("2026-05-15"),orderOrContractNumber:detail("DEMO-2026-001 / DEMO-CONTRACT-001"),usedAmountOrCount:detail("已使用约1个半月"),remainingAmountOrCount:detail("仍有剩余服务，具体待核对"),firstRefundRequestDate:detail("2026-07-01"),merchantResponse:detail("已办理年卡原则上不予退费"),desiredResolution:detail("退还未消费部分费用")});db.prepare("INSERT INTO cases(id,owner_id,title,dispute_type,status,eligibility_json,details_json,is_demo,demo_template_key,demo_setup_state,expires_at,created_at,updated_at)VALUES('demo-case-gym','demo-user','虚构测试案件｜星云健身天府店年卡材料整理','GYM','PENDING_UPLOAD',?,?,1,'prepaid-gym-v1','PARTIAL',?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,details_json=excluded.details_json,is_demo=1,demo_template_key='prepaid-gym-v1',updated_at=excluded.updated_at").run(eligibility,details,new Date(Date.now()+30*86400000).toISOString(),now,now);
console.log("虚构开发账号已创建或更新：");
console.log("- user@demo.local <- DEV_USER_PASSWORD");
console.log("- reviewer@demo.local <- DEV_REVIEWER_PASSWORD");
console.log("- admin@demo.local <- DEV_ADMIN_PASSWORD");
console.log("密码值仅来自 .env.local；环境变量变更后必须重新运行本命令。");
