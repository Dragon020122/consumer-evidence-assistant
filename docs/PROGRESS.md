...
- 阶段 19 作品截图收口：人工提供的 6 张桌面端和 2 张移动端截图均可解析为 JPEG，尺寸为 1920px 宽桌面端和约 372px 宽移动端，单张 46–193 KB；检查未发现真实隐私、密码、本地路径或错误浮层。为避免 `.png.jpg` 双扩展名混淆，保留原有无损内容并统一为 `.jpg` 文件名，README 已展示并完成链接校验；许可证决定为暂不添加 LICENSE，仅作为公开求职作品展示。
...
- 阶段 16 审计完成：记录演示数据、字段状态、上传体验、软阻断、八步导航和 E2E 覆盖缺口；确认现有服务端角色、所有权、分配、文件校验与私有下载边界应原样保留。详见 `docs/archive/UX_AUDIT.md` 与 ADR-006 至 ADR-008。
