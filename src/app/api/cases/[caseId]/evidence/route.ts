...
export async function GET(_: Request, context: { params: Promise<{ caseId: string }> }) {
  try { const actor = await requireSession(); const { caseId } = await context.params; return Response.json({ evidence: listEvidence(actor, caseId).map(toEvidenceFileDTO) }); }
...
export async function POST(request: Request, context: { params: Promise<{ caseId: string }> }) {
  try {
    ...
    return Response.json({ evidence: toEvidenceFileDTO(await storeEvidence(actor, caseId, file, category as never)) }, { status: 201 });
  } catch (error) { return errorResponse(error); }
}
