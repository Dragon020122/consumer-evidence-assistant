...
function confirmedValues(extractions:ExtractionRow[],fieldName:string):Array<string|number>{return extractions.filter(x=>x.fieldName===fieldName&&["CONFIRMED","MODIFIED"].includes(x.state)&&x.confirmedValueJson!==null).map(x=>JSON.parse(x.confirmedValueJson!) as string|number);}
...
if(details.merchantLegalName?.state!=="CONFIRMED"||!details.merchantLegalName?.value)add({code:"MERCHANT_UNCLEAR",priority:"PRIORITY",title:"商家主体尚未明确",recommendation:"建议核对合同、付款收款方、发票或商家公示信息中的主体名称。",basis:"商家主体字段未确认。"});
if(details.storeName?.state==="CONFIRMED"&&details.merchantLegalName?.state==="CONFIRMED"&&details.storeName.value!==details.merchantLegalName.value)add({code:"STORE_ENTITY_RELATION",priority:"OPTIONAL",title:"门店名称与主体名称不同",recommendation:"建议补充能说明门店与公司主体关系的订单、合同或公示信息。",basis:"已确认的门店名称和商家主体名称不一致；这只是材料核对提示。"});
